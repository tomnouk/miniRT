...
/*   Updated: 2024/09/10 14:21:54 by aeid             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool assign_orientation_camera(char *line, t_elem *elem, int *status)
{
	...
	ft_free_split(split_orientation);
	if (elem->orientation.x < -1 || elem->orientation.x > 1 || elem->orientation.y < -1 || elem->orientation.y > 1 || elem->orientation.z < -1 || elem->orientation.z > 1)
	{
		(*status)++;
		printf("%s", ERROR_CAMERA_ORIENTATION);
		return (true);
